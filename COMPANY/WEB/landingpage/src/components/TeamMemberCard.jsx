...
const TeamMemberCard = memo(
  ({ name, role, bio, image, delay = 0 }) => {
    return (
      <motion.div
        ...
        viewport={{ once: true }}
      >
        <div className="w-24 h-24 bg-brand-primary rounded-full mx-auto mb-4 flex items-center justify-center flex-shrink-0 overflow-hidden">
          {image ? (
            <img
              src={image}
              alt={name}
  ...
  (prevProps, nextProps) => {
    // Custom equality check prevents re-renders when props haven't changed
    return (
